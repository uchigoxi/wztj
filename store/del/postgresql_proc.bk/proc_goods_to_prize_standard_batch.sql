CREATE OR REPLACE FUNCTION proc_goods_to_prize_standard_batch(
  ...
  p_remark prize_standard.remark%TYPE
)
  RETURNS integer[] AS
$$
declare
  ...
  v_return_id integer[];
begin
  --select count(*) into STRICT v_tmp from activity_standard where id = p_aid and status = 0;
  select count(*) into v_tmp from activity_standard where id = p_aid and status = 0;
  IF v_tmp = 0 THEN
    RAISE EXCEPTION 'aid % not found', p_aid;
  END IF;
  select count(*) into v_tmp from prize_standard where prize_level = p_prize_level and aid = p_aid;
  IF v_tmp > 0 THEN
  ...
  for v_goods in select * from goods_pool where name = p_gname and status = 0 limit p_num for update loop
    ...
    insert into prize_standard (aid,status,create_date,gid,gname,description,sn,bid,pic,category,prize_name,prize_level,remark)
