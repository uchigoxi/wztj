CREATE OR REPLACE FUNCTION proc_goods_to_prize_business_batch(
  p_aid activity_business.id%TYPE,
  p_gname goods_pool.name%TYPE,
  p_num integer,
  p_use_for goods_pool.use_for%TYPE,
  p_goods_status goods_pool.status%TYPE,
  p_prize_name prize_business.prize_name%TYPE,
  p_prize_level prize_business.prize_level%TYPE,
  p_prize_status prize_business.status%TYPE,
  p_remark prize_business.remark%TYPE
)
  RETURNS integer[] AS
$$
declare
  ...
  v_return_id integer[];
begin
  --select count(*) into STRICT v_tmp from activity_business where id = p_aid and status = 0;
  select count(*) into v_tmp from activity_business where id = p_aid and status = 0;
  IF v_tmp = 0 THEN
    RAISE EXCEPTION 'aid % not found', p_aid;
  END IF;
  select count(*) into v_tmp from prize_business where prize_level = p_prize_level and aid = p_aid;
  IF v_tmp > 0 THEN
  ...
  for v_goods in select * from goods_pool where name = p_gname and status = 0 limit p_num for update loop
    ...
    insert into prize_business (aid,status,create_date,gid,gname,description,sn,bid,pic,category,prize_name,prize_level,remark)
